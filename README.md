## Transcribe AWS course

Using OpenAI whisper model so you can only talk in whispers pspspspspspspspspspspspspspspspspspsp

**Follow-Along Analysis**

- **Source:** `data/misc/timestamps.txt` — parsed consecutive HH:MM:SS timestamps and classified a section as "Follow along" when the label contains the substring "follow" (case-insensitive).
- **Follow along total:** 28:53:30 (28.8917 hours)
- **Non-Follow total:** 12:34:32 (12.5756 hours)
- **Combined total:** 41:28:02

**Top follow-along sessions (longest first)**
- **Start `0:54:11`**: `S3 - CLI Follow Along` — 04:12:21 (~4.206 h)
- **Start `33:06:27`**: `EB VM Follow Along` — 01:26:03 (~1.434 h)
- **Start `30:29:41`**: `AWS Lambda Follow Along` — 01:20:03 (~1.334 h)
- **Start `12:26:09`**: `VPC Follow Along` — 01:15:15 (~1.254 h)
- **Start `14:12:28`**: `NACL Follow Along` — 00:58:12 (~0.970 h)

If you want a CSV of all follow-along intervals or a different matching rule (exact "Follow Along", include "Lab", etc.), tell me which and I will regenerate and add it to the repo.
